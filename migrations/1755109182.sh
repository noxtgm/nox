...
# Users can still opt-in to Cloudflare DNS using: nox-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-nox-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-nox-dns.network
  ...
  echo "To use Cloudflare DNS, run: nox-setup-dns Cloudflare"
fi
