...
if command -v limine &>/dev/null; then
  sudo tee /etc/mkinitcpio.conf.d/nox_hooks.conf <<EOF >/dev/null
...
#timeout: 3
default_entry: 2
interface_branding: nox Bootloader
...
if [ -n "$EFI" ] && efibootmgr &>/dev/null && ! efibootmgr | grep -q nox &&
